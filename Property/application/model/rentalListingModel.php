...
class RentalListingModel {
    ...
    public function getPostedPropertiesWithCount(int $countLimit) {

        $sql = "SELECT * FROM property LIMIT " . $countLimit;
        $query = $this->db->prepare($sql);
        $query->execute();
...
//        }
    ...
    public function getListingsWithParameter($title, $address, $zipcode, $rentFrom, $rentTo, $roomatesFrom, $roomatesTo, $animals) {
        $sql = "SELECT * FROM property WHERE 1";

        if ($title != NULL && trim($title) != '') {
            $sql .= " AND title LIKE '%" . trim($title) . "%'";

//            echo "\n".$sql;
//            return;
        }
        if ($address != NULL && trim($address) != '') {
            $sql .= " AND address LIKE '%" . trim($address) . "%'";
        }
        if ($zipcode != NULL && trim($zipcode) != '') {
            $sql .= " AND zip_code = " . trim($zipcode) . "";
        }
        if ($rentFrom != NULL && trim($rentFrom) != '') {
            $sql .= " AND price >= " . trim($rentFrom) . "";
        }
        if ($rentTo != NULL && trim($rentTo) != '') {
            $sql .= " AND price <= " . trim($rentTo) . "";
        }
        if ($roomatesFrom != NULL && trim($roomatesFrom) != '') {
            $sql .= " AND number_occupants >= " . trim($roomatesFrom) . "";
        }
        if ($roomatesTo != NULL && trim($roomatesTo) != '') {
            $sql .= " AND number_occupants <= " . trim($roomatesTo) . "";
        }
        if ($animals != NULL && trim($animals) != '') {
            $sql .= " AND is_pet_allowed = '" . trim($animals) . "'";
        }
